...
_AI_EXECUTOR = ThreadPoolExecutor(
    max_workers=getattr(settings, 'AI_MAX_WORKERS', 3),
...
class OllamaClient:
    ...
    def generate_response(
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=timeout) as resp:
                for line in resp:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to decode JSON line: {line[:100]}... Error: {e}")
                        continue
            ...
            logger.error(f"Unexpected error during AI generation for chat {chat_id}: {e}", exc_info=True)
...
class ServiceChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        try:
            ...
            if not chat_exists:
                logger.warning(f"User {self.user.id} has no access to chat {self.chat_id}")
    ...
    async def disconnect(self, close_code):
        ...
        if hasattr(self, "room_group_name") and self.room_group_name:
            try:
                ...
                logger.info(f"User {self.user.id if self.user else 'unknown'} "
        ...
        try:
            ...
            await self.channel_layer.group_send(
                self.room_group_name,
                {
                    ...
                    "user_id": str(self.user.id),
                    "chat_id": self.chat_id,
                    "timestamp": user_msg.created_at.isoformat() if hasattr(user_msg, 'created_at') else None
                }
            )
            ...
            logger.info(f"Started AI generation for chat {self.chat_id}, "
...
def handle_ai_errors(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            logger.error(f"Unhandled error in AI function {func.__name__}: {e}", exc_info=True)
            raise
    return wrapper
