...
class TagSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Tag
        fields = ('id', 'name', 'color', 'description', 'parent', 'children_count', 'user')
...
class AreaSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Area
        fields = ('id', 'name', 'description', 'priority', 'color', 
                 'nodes_count', 'projects_count', 'user')
...
class InformationNodeListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'title', 'stage', 'stage_display', 'node_type', 
                 'type_display', 'atomicity_score', 'clarity_score',
                 'tags', 'areas', 'created_at', 'updated_at', 'last_reviewed',
                 'incoming_links_count', 'outgoing_links_count', 'user')
...
class InformationNodeDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = InformationNode
        fields = ('id', 'title', 'content', 'stage', 'stage_display', 
                 'node_type', 'type_display', 'atomicity_score', 'clarity_score',
                 'tags', 'areas', 'sources', 'tag_ids', 'area_ids', 'source_ids',
                 'created_at', 'updated_at', 'last_reviewed', 'user')
        read_only_fields = ('user', 'created_at', 'updated_at')
...
class NodeLinkSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = NodeLink
        fields = ('id', 'from_node', 'from_node_title', 'to_node', 'to_node_title',
                 'link_type', 'link_type_display', 'strength', 'description',
                 'created_at')
...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'title', 'description', 'status', 'status_display',
                 'deadline', 'tags', 'areas', 'nodes_count',
...
class ProcessingSessionSerializer(serializers.ModelSerializer):
    ...
    new_stage_display = serializers.CharField(source='get_new_stage_display', read_only=True)

    class Meta:
        model = ProcessingSession
        fields = ('id', 'node', 'node_title', 'previous_stage', 'previous_stage_display',
                 'new_stage', 'new_stage_display', 'actions_taken', 'time_spent',
                 'insights_gained', 'created_at', 'user')
...
class ProjectContributionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'node', 'node_title', 'project', 'project_title',
                 'role', 'importance', 'added_at')
...
class DailyReviewSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = DailyReview
        fields = ('id', 'date', 'insights', 'completed',
                 'nodes_reviewed_count', 'average_recall_score',
                 'user')
...
class NodeReviewSerializer(serializers.ModelSerializer):
    node_title = serializers.CharField(source='node.title', read_only=True)
    recall_score_display = serializers.CharField(source='get_recall_score_display', read_only=True)

    class Meta:
        model = NodeReview
        fields = ('id', 'review', 'node', 'node_title', 'recall_score',
                 'recall_score_display', 'new_connections', 'reviewed_at')
